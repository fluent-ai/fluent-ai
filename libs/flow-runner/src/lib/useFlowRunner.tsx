...
export const useFlowRunner = (): {
  executeFlow: ({
    ...
    inputs,
    globals,
  }: IExecuteFlowArguments) => Promise<void>;
  executeBranch: ({
    flow,
    inputs,
    globals,
    msg,
    nodeId,
  }: IExecuteBranchArguments) => Promise<void>;
  ...
  const buildRelationships = (flow: IFlow | undefined): IRelationships[] => {
    if (!flow) {
      console.warn(`🌊🪝🚨 Cannot build relationships, flow is falsy`);
      return [];
    }
  ...
  const executeNode = ({
    ...
    return new Promise((resolve) => {
      ...
      let method:
        | ((args: IMethodArguments) => Promise<Record<string, unknown>>)
        | undefined;
      try {
      ...
      if (method) {
        ...
        method({
          globals,
          inputs:
            inputs.find((input) => input.id === node.id)?.nodeInputs || {},
          ...
          .then((msg) => {
            ...
            if (msg.error) {
              console.warn(
                `🌊🪝🚨 Error executing node ${node.id}. Stopping Branch`
              );
            ...
            relationships
              .find((nodeChildren) => nodeChildren.id === node.id)
              ?.nodeChildren.forEach((childId) => {
                const childNode = findNode(flow?.nodes, childId);
                if (childNode) {
                  if (childNode.type === 'runner') {
                    console.log(
                      `🌊🪝 ${childId} is a runner, pausing for manual continue`
                    );
                  } else {
                    childPromises.push(
                      executeNode({
  ...
  const executeFlow = async ({
    ...
    const rootNodes = flow?.nodes.filter((node) =>
      isRootNode(flow?.edges, node.id)
    );
    // Start the execution by triggering executeNode on each root
    if (rootNodes) {
      const promises = rootNodes.map((rootNode) =>
        executeNode({
          flow,
          relationships,
          node: rootNode,
