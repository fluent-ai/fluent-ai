...
function ConditionDialog({ nodeId }: { nodeId: string }) {
  ...
  if (inputs?.location === undefined) {
    dispatch(
      flowActions.setInput({
        id: nodeId,
        nodeInputs: { ...inputs, location: 'msg.payload' },
      })
    );
  }
  if (inputs?.operator === undefined) {
    dispatch(
      flowActions.setInput({
        id: nodeId,
        nodeInputs: { ...inputs, operator: 'is' },
      })
    );
  }
  if (inputs?.query === undefined) {
    dispatch(
      flowActions.setInput({ id: nodeId, nodeInputs: { ...inputs, query: '' } })
    );
  }
  ...
  return (
    <InnerDialogStructure title="Condition" description="Condition test">
      <div title="Condition">
