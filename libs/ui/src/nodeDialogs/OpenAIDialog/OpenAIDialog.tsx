...
function OpenAIDialog({ nodeId }: { nodeId: string }) {
  ...
  const modes = [
    {
      ...
      description: (
        <p>
          Returns the response as a string in <code>msg.payload</code>
        </p>
      ),
    },
    {
      ...
      description: (
        <div>
          <p>
            The node expects msg.payload to follow openAI's
            <a
            ...
            . New messages are appended to the messages property, allowing you
            to chain nodes to form a conversation
          </p>
        </div>
      ),
    },
  ...
  return (
    <InnerDialogStructure title="OpenAI" description="OpenAI description">
      {error && (
      ...
      <div title="Options">
        <b>Input</b>
        <p>
          The openAI node expects a payload on the msg object (as most nodes).{' '}
          <code>msg.payload</code> can be either a string or an object.
        </p>
        <br />
        <RadioGroup
          title="Mode"
