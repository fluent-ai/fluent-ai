...
function TemplateDialog({ nodeId }: { nodeId: string }) {
  ...
  const onChange: React.ChangeEventHandler<HTMLTextAreaElement> = (event) => {
    setPreview(
      Mustache.render(event.target.value, {
        globals,
        inputs,
        msg: state?.inputMsg,
      })
  ...
  return (
    <InnerDialogStructure title="Template">
      <div title="Info">
        Use <b>double braces</b>, ie. {' {{ }} '} to insert data into your
        template.
        <br /> The template node uses the data from the previous node
        <br />
        Place the variable name inside the braces and it will be replaced with
        the value.
        <br />
        For example
      ...
      <div title="Template">
        ...
        <div>
          ...
          <p>
            Note : The preview runs on the last data the node received and may
            differ from the output at runtime
          </p>
          <pre className="text-sm p-2">
            <code>{preview}</code>
          </pre>
