...
const NodeSideBar = () => {
  ...
  <aside className={`px-2 py-2.5 rounded-md bg-white  border-2 border-inherit ${open ? 'w-60': 'w-12 overflow-hidden'}`}>
    ...
    <NodeItemSideBar classes="input" onDragStartHandler={(event) => onDragStart(event, 'input')} title="Files" icon={<FileIcon />} />
    <NodeItemSideBar onDragStartHandler={(event) => onDragStart(event, 'templateNode')} title="Template" icon={<DoubleArrowRightIcon />} />
    <NodeItemSideBar onDragStartHandler={(event) => onDragStart(event, 'default')} title="Default" icon={<MixIcon />}/>
    <NodeItemSideBar onDragStartHandler={(event) => onDragStart(event, 'output')} title="Output" icon={<DoubleArrowRightIcon />} />
  </aside>
  {open ?
  <div
